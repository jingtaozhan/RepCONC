...
def get_collate_function(max_seq_length):
    ...
    def collate_function(batch):
        # force peak cuda memory usage at beginning
        # to avoid outofmemory error in the middle of training
        nonlocal cnt 
        length = None
        if cnt < 10:
...
def triple_get_collate_function(max_query_length, max_doc_length, rel_dict, padding=False):
    ...
    def collate_function(batch):
        query_data, query_ids = query_collate_func([x[0] for x  in batch])
        # doc_data, doc_ids = doc_collate_func([x[1] for x in batch])
        doc_data, doc_ids = doc_collate_func(sum([x[1] for x in batch], []))
        query_num = len(query_data['input_ids'])
        doc_num_per_query = len(batch[0][1])

        scores_mask = [[1 if docid in rel_dict[qid] else 0 
            for docid in doc_ids] for qid in query_ids]
        scores_mask = torch.FloatTensor(scores_mask)
        
        labels = torch.arange(0, len(doc_ids), doc_num_per_query)
        assert torch.all(scores_mask[torch.arange(query_num), labels].int() == 1)
        scores_mask[torch.arange(query_num), labels] = 0
